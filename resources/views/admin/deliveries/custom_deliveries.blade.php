...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark">Custom Orders Delivery</h1>
                    <p>list deliveries of custom orders</p>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    </div>
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab" style="margin: 1%">
                                    <table id="orderTable" class="table table-hover">
                                        ...
                                        @foreach($deliveries as $delivery)
                                            @if($delivery->status == 'On Delivery' && $delivery->custom_order->status != 2)
                                                <tr>
                                                    <td>{{$delivery->id}}</td>
                                                    <td>{{$delivery->customer_name}}</td>
                                                    <td>{{$delivery->custom_order->city}}</td>
                                                    <td>{{$delivery->custom_order->address}}</td>
                                                    ...
                                                    <td>
                                                        <a href="{{route('custom_deliveries.status', ['status' => 'Completed', 'id' => $delivery->id])}}" class="btn btn-lg btn-success">Complete</a>
                                                        <a href="{{route('custom_deliveries.status', ['status' => 'Return', 'id' => $delivery->id])}}" class="btn btn-lg btn-primary">Return</a>
                                                    </td>
                                ...
                                <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab" style="margin: 1%">
                                    <table id="atkTable" class="table table-hover">
                                        ...
                                        <tbody>
                                        @foreach($deliveries as $delivery)
                                            @if($delivery->status == 'Return' && $delivery->custom_order->status != 2)
                                                <tr>
                                                    <td>{{$delivery->id}}</td>
                                                    <td>{{$delivery->customer_name}}</td>
                                                    <td>{{$delivery->custom_order->city}}</td>
                                                ...
                                                <div class="modal fade" id="deliver{{$delivery->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                                    <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered" role="document">
                                                        <div class="modal-content">
                                                            <form action="{{route('custom_deliveries.edit_eda', $delivery->id)}}" class="form-horizontal" method="POST">
                                                                @csrf
                                                                    ...
                                                                    <div class="row">
                                                                        <div class="col">
                                                                            <div class="form-group">
                                                                                ...
                                                                                <div class="col-md-11">
                                                                                    <input name="date" id="datepicker{{$delivery->id}}" type="text" class="form-control input-md" value="">
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                ...
                                <div class="tab-pane fade" id="nav-contact" role="tabpanel" aria-labelledby="nav-contact-tab" style="margin: 1%">
                                    <table id="defTable" class="table table-hover">
                                        ...
                                        <tbody>
                                        @foreach($deliveries as $delivery)
                                            @if($delivery->status == 'Completed' && $delivery->custom_order->status != 2)
                                                <tr>
                                                    <td>{{$delivery->id}}</td>
                                                    <td>{{$delivery->customer_name}}</td>
                                                    <td>{{$delivery->custom_order->city}}</td>
